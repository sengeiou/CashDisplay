...
public class AsyncServer {
    ...
    private static final int MAX_COUNT_ADRESS = 10;

    private static final long MIN_HEAP_SIZE = (long)(MemoryClass.getMaxHeapSize()/4);//5

    private static ArrayList mlistInetAdressConnections = new ArrayList<String>(MAX_COUNT_ADRESS+1);//список адресов, которые подключались
...
//        Log.i(LOGTAG, "****AsyncServer is starting.****");
        ...
        while(true) {
            ...
            synchronized (server) {
                if (selector.isOpen() && (selector.keys().size() > 0 || queue.size() > 0)) {
                    //Log.w(LOGTAG, "AsyncServer runLoop");
                    continue;
                }
        ...
        try {
            ...
            if (needsSelect) {
                if (wait == QUEUE_EMPTY) {
                    ...
                    Log.i(LOGTAG, "nothing to select immediately but there's something pending so let's block that duration and wait.");
            ...
            try {
                if (key.isAcceptable()) {
                    ...
                    try {
                        ...
                        //анализ IP адресов, с которых выполняются запросы
                        mlistInetAdressConnections.add(sc.socket().getInetAddress().toString().substring(1));
                        if (mlistInetAdressConnections.size()>MAX_COUNT_ADRESS)
                        {
                            mlistInetAdressConnections.remove(0);
                        }

                        //Log.i(LOGTAG, "isAcceptable");
                        int count = Collections.frequency(mlistInetAdressConnections, sc.socket().getInetAddress().toString().substring(1));

                        //Log.i(LOGTAG, "readyKeys.size(): "+readyKeys.size()+" :"+sc.socket().getInetAddress().toString().substring(1)+", count:"+count+", availHeapSizeInMB:"+availHeapSizeInMB);
                        long availheap = MemoryClass.getAvailableHeapSize();

                       // if ((count>=MAX_COUNT_ADRESS)&&(availheap<MIN_HEAP_SIZE))
                        if (availheap<MIN_HEAP_SIZE)
                        {
                           // sc.socket().close();
                           // sc.close();
                            ...
                            Log.w(LOGTAG, "Reset incoming connection, free memory:"+availheap+"MB, MIN_HEAP_SIZE: "+MIN_HEAP_SIZE);
                           continue;
                        }
                    ...
                    //сброс входящих пакетов, из-за отсутствия памяти
                    long availheap = MemoryClass.getAvailableHeapSize();
                    if (availheap<MIN_HEAP_SIZE) {
                        //Log.w("tag", "availHeapSizeInMB="+availHeapSizeInMB);

                        if (key != null)
                        key.cancel();
                        continue;
                    }
                     //Log.i(LOGTAG, "isReadable");
    ...
    private static void setIvent_HaltServer(){

        Log.i(LOGTAG, "setIvent_HaltServer");
        Intent intent = new Intent(HTTP_HALT_EVENT);
        AsyncHttpServer.mContext.sendBroadcast(intent);
    }
